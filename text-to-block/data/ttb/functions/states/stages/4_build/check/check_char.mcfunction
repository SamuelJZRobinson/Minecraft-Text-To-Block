...
execute if score noCharMatch StampFont matches 0 run schedule function ttb:states/stages/4_build/place/manage_place_char 5t replace
...
# Fail Loop
execute if score noCharMatch StampFont matches 1 if score charID StampFont < MAX_CHAR_ID StampFont run function ttb:states/stages/4_build/check/check_char_fail